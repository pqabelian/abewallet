...
func (w *Wallet) ImportP2SHRedeemScript(script []byte) (*abeutil.AddressScriptHash, error) {
	var p2shAddr *abeutil.AddressScriptHash
	err := walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		...
		if err != nil {
			...
			if waddrmgr.IsError(err, waddrmgr.ErrDuplicateAddress) {
				...
				p2shAddr, _ = abeutil.NewAddressScriptHash(script,
					w.chainParams)
				return nil
			}
			return err
		...
		p2shAddr = addrInfo.Address().(*abeutil.AddressScriptHash)
		return nil
	})
	return p2shAddr, err
}
