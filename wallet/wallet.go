...
func newFilterBlocksRequest(batch []wtxmgr.BlockMeta,
	...
	filterReq := &chain.FilterBlocksRequest{
		Blocks:           batch,
		ExternalAddrs:    make(map[waddrmgr.ScopedIndex]abeutil.Address),
		InternalAddrs:    make(map[waddrmgr.ScopedIndex]abeutil.Address),
...
func (w *Wallet) CurrentAddress(account uint32, scope waddrmgr.KeyScope) (abeutil.Address, error) {
	chainClient, err := w.requireChainClient()
	if err != nil {
		return nil, err
	}
	...
	if props != nil {
		err = chainClient.NotifyReceived([]abeutil.Address{addr})
		if err != nil {
			return nil, err
		}
...
func (w *Wallet) PubKeyForAddress(a abeutil.Address) (*btcec.PublicKey, error) {
...
// address.
func (w *Wallet) PrivKeyForAddress(a abeutil.Address) (*btcec.PrivateKey, error) {
...
func (w *Wallet) AccountOfAddress(a abeutil.Address) (uint32, error) {
	var account uint32
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		var err error
...
func (w *Wallet) Accounts(scope waddrmgr.KeyScope) (*AccountsResult, error) {
	...
	err = walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		m := make(map[uint32]*abeutil.Amount)
...
func (w *Wallet) DumpPrivKeys() ([]string, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		// privkeys.
		return w.Manager.ForEachActiveAddress(addrmgrNs, func(addr abeutil.Address) error {
...
func (w *Wallet) DumpWIFPrivateKey(addr abeutil.Address) (string, error) {
...
func (w *Wallet) ImportPrivateKey(scope waddrmgr.KeyScope, wif *abeutil.WIF,
	...
	if rescan {
		...
		err := w.chainClient.NotifyReceived([]abeutil.Address{addr})
		if err != nil {
...
func (w *Wallet) NewAddress(account uint32,
	scope waddrmgr.KeyScope) (abeutil.Address, error) {
...
func (w *Wallet) newAddress(addrmgrNs walletdb.ReadWriteBucket, account uint32,
	scope waddrmgr.KeyScope) (abeutil.Address, *waddrmgr.AccountProperties, error) {
...
func (w *Wallet) newChangeAddress(addrmgrNs walletdb.ReadWriteBucket,
	account uint32, scope waddrmgr.KeyScope) (abeutil.Address, error) {
...
func (w *Wallet) SendOutputs(outputs []*wire.TxOut, account uint32,
	minconf int32, satPerKb abeutil.Amount, label string) (*wire.MsgTx, error) {
...
func (w *Wallet) SendOutputsAbe(outputDescs []*abecrypto.AbeTxOutputDesc, minconf int32, feePerKbSpecified abeutil.Amount, feeSpecified abeutil.Amount, utxoSpecified []string, label string) (*wire.MsgTxAbe, error) {
	...
	txHash, err := w.reliablyPublishTransactionAbeNew(createdTx.Tx, label)
	if err != nil {
		return nil, err
	}
...
func (w *Wallet) reliablyPublishTransactionAbeNew(tx *wire.MsgTxAbe,
	label string) (*chainhash.Hash, error) {
...
func (w *Wallet) publishTransactionAbe(tx *wire.MsgTxAbe) (*chainhash.Hash, error) {
	...
	dbErr := walletdb.Update(w.db, func(dbTx walletdb.ReadWriteTx) error {
		...
		w.TxStore.RemoveUnminedTxAbe(txmgrNs, txRec)
		return nil
	})
	if dbErr != nil {
		log.Warnf("Unable to remove invalid transaction %v: %v",
