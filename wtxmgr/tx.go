...
func (s *Store) InsertBlockAbeNew(txMgrNs walletdb.ReadWriteBucket, block *BlockAbeRecord, maturedBlockHashs []*chainhash.Hash, addrToVskMap map[string][]byte, coinAddrToInstanceAddr map[string][]byte) error {
	...
	if block.Height%3 == 2 {
			...
			for _, outpoint := range utxoRingEntry.OutPointRing().OutPoints {
				...
				if utxo != nil { // this ring has utxo belonging to the wallet
					...
					if !ok1 {
						...
						// put the ring to coinbase
						err := putRingDetails(txMgrNs, ringHash[:], ring.Serialize()[:])
						if err != nil {
							return err
						}
		...
		for ringHash, utxoRing := range willAddUTXORing {
			...
			err = putRawNeedUpdateUTXORing(txMgrNs, ringHash[:], []byte{1})
			if err != nil {
				return err
			}
		}
	...
	for i := maxHeight; i > height; i-- {
		...
		if i%blockNumOfRing == blockNumOfRing-1 {
			...
			for j := int32(1); j < blockNumOfRing; j++ {
				...
				trOutput, err := fetchImmaturedOutput(ns, i-j, *blockHash)
				if err != nil {
					return err
				}
				if cbOutput != nil || trOutput != nil {
					err = deleteImmaturedCoinbaseOutput(ns, keysToRemove[i-j])
					if err != nil {
						return err
					}
					err = deleteImmaturedOutput(ns, keysToRemove[i-j])
					if err != nil {
						return err
					}
				} else {
					...
					for k := 0; k < len(outpoint); k++ {
						...
						if err == nil { // this output is in unspent txo bucket
							...
							err := putRawMaturedOutput(ns, key, newV)
							if err != nil {
							...
							if err == nil {
								...
								err = putRawMaturedOutput(ns, key, newV) // move to unspent txo bucket
								if err != nil {
									return fmt.Errorf("putRawUnspentTXO error in rollbackAbe : %v", err)
								}
