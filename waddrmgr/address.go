...
//
// This is part of the ManagedAddress interface implementation.
func (a *managedAddress) Address() abeutil.Address {
	return a.address
}
...
//
// This is part of the ManagedPubKeyAddress interface implementation.
func (a *managedAddress) ExportPrivKey() (*abeutil.WIF, error) {
	pk, err := a.PrivKey()
	if err != nil {
		return nil, err
	}

	return abeutil.NewWIF(pk, a.manager.rootManager.chainParams, a.compressed)
}
...
func newManagedAddressWithoutPrivKey(m *ScopedKeyManager,
	...
	case NestedWitnessPubKey:
		...
		witAddr, err := abeutil.NewAddressWitnessPubKeyHash(
		...
		address, err = abeutil.NewAddressScriptHash(
	...
	case PubKeyHash:
		address, err = abeutil.NewAddressPubKeyHash(
			pubKeyHash, m.rootManager.chainParams,
		)
		if err != nil {
			return nil, err
	...
	case WitnessPubKey:
		address, err = abeutil.NewAddressWitnessPubKeyHash(
			pubKeyHash, m.rootManager.chainParams,
		)
		if err != nil {
			return nil, err
...
type scriptAddress struct {
	manager         *ScopedKeyManager
	account         uint32
	address         *abeutil.AddressScriptHash
...
//
// This is part of the ManagedAddress interface implementation.
func (a *scriptAddress) Address() abeutil.Address {
	return a.address
}
...
func newScriptAddress(m *ScopedKeyManager, account uint32, scriptHash,
	...
	address, err := abeutil.NewAddressScriptHashFromHash(
