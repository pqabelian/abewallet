...
func (s *Server) handlerClosure(request *abejson.Request) lazyHandler {
	s.handlerMu.Lock()
...
func sanitizeRequest(r *abejson.Request) string {
...
func (s *Server) invalidAuth(req *abejson.Request) bool {
	cmd, err := abejson.UnmarshalCmd(req)
	if err != nil {
		return false
	}
	authCmd, ok := cmd.(*abejson.AuthenticateCmd)
	if !ok {
		return false
	}
	// Check credentials.
	...
	for {
		...
		case reqBytes, ok := <-wsc.allRequests:
			...
			default:
				...
				go func() {
					resp, jsonErr := f()
					mresp, err := abejson.MarshalResponse(req.ID, resp, jsonErr)
					if err != nil {
...
func (s *Server) postClientRPC(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		resp, err := abejson.MarshalResponse(req.ID, nil, abejson.ErrRPCInvalidRequest)
		if err != nil {
			log.Errorf("Unable to marshal response: %v", err)
	...
	// Marshal and send.
	mresp, err := abejson.MarshalResponse(req.ID, res, jsonErr)
	if err != nil {
		log.Errorf("Unable to marshal response: %v", err)
