...
var (
	walletDataDirectory = abeutil.AppDataDir("btcwallet", false)
	newlineBytes        = []byte{'\n'}
)
...
func makeInputSource(outputs []abejson.ListUnspentResult) txauthor.InputSource {
	var (
		totalInputValue abeutil.Amount
		inputs          = make([]*wire.TxIn, 0, len(outputs))
		inputValues     = make([]abeutil.Amount, 0, len(outputs))
		sourceErr       error
	)
	for _, output := range outputs {
		outputAmount, err := abeutil.NewAmount(output.Amount)
		if err != nil {
	...
	return func(abeutil.Amount) (abeutil.Amount, []*wire.TxIn, []abeutil.Amount, [][]byte, error) {
...
func sweep() error {
	...
	sourceOutputs := make(map[string][]abejson.ListUnspentResult)
	...
	for _, previousOutputs := range sourceOutputs {
		...
		outputAmount := abeutil.Amount(tx.Tx.TxOut[0].Value)
...
func saneOutputValue(amount abeutil.Amount) bool {
	return amount >= 0 && amount <= abeutil.MaxSatoshi
}

func parseOutPoint(input *abejson.ListUnspentResult) (wire.OutPoint, error) {
